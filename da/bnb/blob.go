...
func (b *Blob) ComputeKZGCommitment() (Commitment, error) {
	return BlobToCommitment(*b.KZGBlob())
}
...
func KZGToVersionedHash(commitment Commitment) (out common.Hash) {
	hasher := sha256.New()
	return CalcBlobHashV1(hasher, &commitment)
}
...
func VerifyBlobProof(blob *Blob, commitment Commitment, proof Proof) error {
	return VerifyBlobProofkzg(*blob.KZGBlob(), commitment, proof)
}
...
var (
	minBlobGasPrice            = big.NewInt(BlobTxMinBlobGasprice)
