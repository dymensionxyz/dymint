...
func (txmp *TxMempool) initialTxCallback(wtx *WrappedTx, res *abci.Response) {
	...
	if sender != "" {
		...
		if ok {
			...
			checkTxRes.CheckTx.MempoolError = fmt.Sprintf(
				"rejected valid incoming transaction; tx already exists for sender %q (%X)",
				sender,
				w.tx.Hash(),
			)
	...
	if err := txmp.canAddTx(wtx); err != nil {
		...
		if len(victims) == 0 || victimBytes < wtx.Size() {
			...
			checkTxRes.CheckTx.MempoolError = fmt.Sprintf(
				"rejected valid incoming transaction; mempool is full (%X)",
				wtx.tx.Hash(),
			)
			txmp.metrics.RejectedTxs.Add(1)
			return
