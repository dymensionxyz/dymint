...
func (m *Manager) checkForkUpdate(ctx context.Context, msg string) error {
	...
	if shouldStopNode(expectedRevision, nextHeight, actualRevision) {
		...
		m.freezeNode(ctx, fmt.Errorf("%s  local_block_height: %d rollapp_revision_start_height: %d local_revision: %d rollapp_revision: %d", msg, m.State.Height(), expectedRevision.StartHeight, actualRevision, expectedRevision.Number))
	}
